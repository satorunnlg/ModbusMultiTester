	...
	public partial class MainForm : Form
	{
		...
		private void AddMonitorWindow()
		{
			...
			child.DataEdited += (addr, val) =>
			{
				...
				{
					// UIスレッドから呼ばれるが、通信はブロックする可能性があるためTaskで逃がすのがベター
					// ただしここでは簡易的に同期呼び出し、またはfire-and-forgetで行う
					Task.Run(() =>
					{
						try
						{
							...
							switch (typeIdx)
							{
								...
								case 3: // Holding Register (Write Single Register 0x06)
									...
									break;
								default:
									// Input(1x), InputReg(3x) はModbus規格上、マスタから書き込めない(Read Only)
									// UI側でエラー表示するなどしても良い
			...
			if (radioButtonMaster.Checked)
			{
				...
				try
				{
					...
					foreach (var monitor in monitors)
					{
						...
						// ★★★ 追加: マスタモードでもグリッド編集を許可する ★★★
						// ただし、Input(1x)とInputRegister(3x)は書き込めないのでFalseにする
