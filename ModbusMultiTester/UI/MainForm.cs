...
namespace ModbusMultiTester
{
	...
	public partial class MainForm : Form
	{
		...
		private void AddMonitorWindow()
		{
			...
			// Master/Slave両モード用のデータ編集イベント登録
			child.DataEdited += async (addr, val) =>
			{
				if (radioButtonSlave.Checked && _mySlave != null)
				{
					// --- スレーブモード：DataStoreに書き込み ---
					try
					{
						...
						AppLogger.Info($"Slave Write: Addr={addr}, Val={val}");
					}
					catch (Exception ex)
					{
				...
				else if (radioButtonMaster.Checked && _modbusMaster != null)
				{
					...
					try
					{
						...
						await Task.Run(() =>
						{
							switch (typeIdx)
							{
								case 0: // Coil (0x) - 書き込み可能
									_modbusMaster.WriteSingleCoil(slaveId, addr, val != 0);
									break;
								case 1: // Discrete Input (1x) - 読み取り専用
									throw new InvalidOperationException("離散入力は読み取り専用です。");
								case 2: // Input Register (3x) - 読み取り専用
									throw new InvalidOperationException("入力レジスタは読み取り専用です。");
								case 3: // Holding Register (4x) - 書き込み可能
									_modbusMaster.WriteSingleRegister(slaveId, addr, val);
									break;
							}
						});

						AppLogger.Info($"Master Write: SlaveID={slaveId}, Addr={addr}, Val={val}");
					}
					catch (Exception ex)
					{
